...
public class  DeleteRecyclerView extends RecyclerView {
    ...
    private ImageView mDeleteView; //删除
    private ImageView mJinDianView;//进店
        ...
        switch (e.getAction()){
            case MotionEvent.ACTION_DOWN:
                ...
                if (status == CLOSE) {
                    ...
                    CommonViewHolder viewHolder = (CommonViewHolder) getChildViewHolder(view);

                    mItemView = viewHolder.getView(R.id.item_layout);
                    mPosition = viewHolder.getAdapterPosition();
                    mDeleteView = viewHolder.getView(R.id.iv_deleted);
                    mJinDianView = viewHolder.getView(R.id.iv_jindian);
                    mMaxLength = mDeleteView.getWidth()+mJinDianView.getWidth();
                    ...
                    mJinDianView.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            //返回原点
                            mItemView.scrollTo(0,0);
                            status = CLOSE;
                            if (mListener!=null)
                                mListener.onJinDianClick(mPosition);
                        }
                    });
